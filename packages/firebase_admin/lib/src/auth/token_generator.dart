...
class FirebaseTokenGenerator {
  ...
  String createCustomToken(String uid, Map<String, dynamic> developerClaims) {
    ...
    var token = builder.build().toCompactSerialization();

    token = _fixKey(claims)!;

    return token;
  }

  String? _fixKey(Map<String, dynamic> claims) {
    final jwt = JWT(claims);

    var servicePrivateKey = json.decode(File(Credentials.serviceAccountPath!)
        .readAsStringSync())['private_key'];

    final key = RSAPrivateKey(servicePrivateKey);

    var modifiedToken = jwt.sign(key, algorithm: JWTAlgorithm.RS256);
